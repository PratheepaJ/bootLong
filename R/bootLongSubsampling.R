...
#' @import "dplyr"
#' @import "parallel"
bootLongSubsampling <- function(ps,R,RR,factors,time,SubjectID_n="SubjectID",lI,omega=.6){
    ...
    mseKhatKobs <- lapply(seq_len(length(lC)),function(y){bootLongMSEPsi(ps=ps,qj=qj,Wj=Wj,b=y,R=R,RR=RR,factors=factors,time=time,Khat.obs=Khat.obs,T.obs.full=T.obs)})
    return(mseKhatKobs)
}
