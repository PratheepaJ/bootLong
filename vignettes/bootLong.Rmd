...
Now we have chosen the initial parameters or the subsampling method. $\omega = .6$ and $l_{I} =5$. 

#   The block bootstrap method

First, we identify the opitmal block size for the subsample. We use the initial block size 5 ($l_{I} = 5$) and subsampling percentage 60\% ($\omega = .6$) to identify the opitmal block size. 

## Subsampling

We can run this code parallel for each block sizes and setting lC1 = lC2 and save the results with the block size (lC1) used.
...
1) Find the optimal block size for the subsample of 60\% (l_{\omega}) using MSE plot. 
