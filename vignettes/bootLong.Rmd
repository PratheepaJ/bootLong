...
The plot can be modified using your preferred theme, legends, etc. For example, `set.theme` can be changed to have point size 10. 
...
## Load the example dataset stored as a `phyloseq` in this package (or use your own `phyloseq` stored or created as a `phyloseq`)
...
In the example data, we have `SubjectID`, `SampleID`, `Time`, `Preterm` variables. `Preterm` is the covariate.

```{r }
names(sample_data(ps))
table(sample_data(ps)$SubjectID, sample_data(ps)$Time)
summary(sample_data(ps)$Preterm)
```
