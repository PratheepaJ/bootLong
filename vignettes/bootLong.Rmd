...
2) We choose top six abundance ASVs (based on the total transformed abundance) to visualize the dependence within-subject. We can choose different ASVs by changing `starttaxa` and `endtaxa`  arguments in `longPACFMultiple()`.
...
Only the largest spike is observed at lag 1 for both term and preterm. However, almost all the ASVs show that the PACF is less than .25 (sufficiently zero) after lag 3. Therefore, the initial block size is 4 (lag 3 corresponds to block size 4). 
...
We observe that for the top abundance ASV, the lag-plot at lag 3 (green) deviates from a linear line. Thus, we choose an initial block size of 4.
...
We observe that the correlogram tends to be sufficiently zero (first time) at lag 3 for almost all the ASVs. Therefore, the initial block size is 4.
...
We cannot get a clear idea about inital block size because subject variability dominates variogram fit.

All other exploratory tools suggest an inital block size of 4.
...
First, we identify the opitmal block size for the subsample. We use the initial block size 4 ($l_{I} =4$) and subsample percentage 60\% ($\omega = .6$) to identify the opitmal block size. 
