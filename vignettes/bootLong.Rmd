...
#sample_data(ps)$GWColl <- as.integer(sample_data(ps)$GWColl)
#sample_data(ps)$SubjectID <- as.character(sample_data(ps)$SubjectID)
...
Only the largest spike is observed at lag 1 for both term and preterm. However, almost all the ASVs show that the PACF is less than .25 (sufficiently zero) after lag 4. Therefore, the initial block size is 5. 
...
We can choose either points (residuals) to be ploted by `point = TRUE`.

```{r message=FALSE, warning=FALSE, fig.width=7.5, fig.height=5.5}
p.all <- longVarioMultiple(ps.tr[[1]],
                           ps.tr[[2]],
                           main_factor = "Seroconversion",
                           time_var="Time",
